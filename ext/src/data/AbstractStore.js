...
Ext.define('Ext.data.AbstractStore', {
    ...
    /**
     * Reloads the store using the last options passed to the {@link #load} method. You can use the reload method to reload the
     ...
     *     store.reload();
     *
     * The initial {@link #load} execution will pass the `userid` parameter in the request. The {@link #reload} execution
     * will also send the same `userid` parameter in its request as it will reuse the `params` object from the last {@link #load} call.
     *
     ...
     *     });
     *
     * The initial {@link #load} execution sends the `userid` and `foo` parameters but in the {@link #reload} it only sends
     ...
     * {@link #load} method for valid configs.
     */
